...
def analyseCsv(filePath, size):
    ...
    with open(filePath) as csvfile:
        ...
        # 找到gap  参考第一列， 找到连续n个为“—begainwith”的起始以及结束行
        for line in fileArray:
            i = fileArray.index(line)
            if fileArray[i][ETtiTraceDlParUe_dataInDrb1_index] != "-":
                continue
            for j in range(n):
        ...
        for indexitem in finalshowindexlist:
            ...
            zerocount = 0
            nonecount = 0
            for count in dataInDrb1Listtemp:
                if count == "0":
                    zerocount = zerocount + 1
                elif count == '-':
                    nonecount = nonecount + 1
            zerorate = round((zerocount * 100)/(len(dataInDrb1Listtemp) - nonecount),2)
            finalshowdata["zerorate"] = zerorate

            dataInDrb1Listtemp = transList(dataInDrb1Listtemp)
            dataInDrb1Listtemp = [x / 1000 * 8  for x in dataInDrb1Listtemp]
            ...
            expected_t_put_listtemp = [x / 1000 /256  for x in expected_t_put_listtemp]
            ...
            for ackitem in EHarqParDl_ackNackDtxCw1_listtemp:
                ...
                if len(ackitem) - blank == 0:
                    ...
                    rate = (count * 100 / (len(ackitem) - blank))
        ...
        finalshowdata["cw1_y"] = [20] * len(cw1)
        finalshowdata["cw2_x"] = cw2
        finalshowdata["cw2_y"] = [-20] * len(cw2)

        drb_ave_List = []
        for temp in splitList(dataInDrb1List, size):
            drb_ave_List.append(np.mean(temp))
        ave_x = list(range(0, len(drb_ave_List)))
        ave_x = [(x + 0.5) * size for x in ave_x]

        finalshowdata["dataInDrb1List_x"] = ave_x
        finalshowdata["dataInDrb1List_y"] = drb_ave_List

        expected_ave_List = []
        for temp in splitList(expected_t_put_list, size):
            expected_ave_List.append(np.mean(temp))
        finalshowdata["expected_t_put_x"] = ave_x
        finalshowdata["expected_t_put_y"] = expected_ave_List

        finalshowdata["EHarqParDl_harqReportSource_x"] = EHarqParDl_harqReportSource_list
        finalshowdata["EHarqParDl_harqReportSource_y"] = [60] * len(EHarqParDl_harqReportSource_list)
    ...
    tput_tti_List = [x / 1000 * 8 for x in tput_tti_List]

    tput_ave_List = []
    for temp in splitList(tput_tti_List, size):
        tput_ave_List.append(np.mean(temp))
    tput_x = list(range(0, len(tput_ave_List)))
    tput_x = [(x + 0.5) * size for x in tput_x]

    finalshowdata["tput_x"] = tput_x
    finalshowdata["tput_y"] = tput_ave_List
    ...
    filePath = filePath.replace(".csv", "_" + str(size) + ".txt")
...
def readAndShow(filepath):
    ...
    with open(filepath, 'r') as f:
        ...
        plt.plot(readoutdataDict["dataInDrb1List_x"], readoutdataDict["dataInDrb1List_y"], label='dataInDrb1, 0 proportion:' + str(readoutdataDict["zerorate"]) + "%", marker='.')
        plt.plot(readoutdataDict["expected_t_put_x"], readoutdataDict["expected_t_put_y"], label='expectedThroughput', marker='.')
        if "EHarqParDl_harqReportSource_x" in  readoutdataDict:
             plt.bar(readoutdataDict["EHarqParDl_harqReportSource_x"], readoutdataDict["EHarqParDl_harqReportSource_y"], label='harqReportSource_timeout',fc="r", alpha=0.8)

        plt.plot(readoutdataDict["ack_rate_x"], readoutdataDict["ack_rate_y"], label='ack rate', marker='.')
        plt.bar(readoutdataDict["cw1_x"], readoutdataDict["cw1_y"], label='missing cw1', fc="orange", alpha=0.8)
        plt.bar(readoutdataDict["cw2_x"], readoutdataDict["cw2_y"], label='missing cw2', fc="k", alpha=0.8)
        plt.plot(readoutdataDict["tput_x"], readoutdataDict["tput_y"], label='thoughtput', marker='.')
...
if __name__ =="__main__":
    ...
    analyseCsv(filepath, 100)
    readAndShow(filepath.replace(".csv", "_100.txt"))
